# UFFT Paper #59 — Paper #59 — The Central Theorem: From Foam to the Standard Model
...
The factor (−1) is the A₂u torsion eigenvalue. The Yukawa coupling for each generation is:
...
The A₂u mode (eigenvalue 9, torsion eigenvalue −1) generates the Higgs potential:

**V(φ) = μ²|φ|² + λ|φ|⁴**

where:
- μ² = −(λ_{A₂u} × T_{A₂u}) × M_P² × exp(−2S_H) = negative (because T_{A₂u} = −1)
- λ = 1/F_hx = 1/8 = 0.125 (tree-level, from the A₂u ⊗ A₂u → A₁g channel)

The negative μ² is not a parameter choice. It is a geometric theorem: the A₂u mode is the UNIQUE mode with negative torsion eigenvalue (Paper #57, Theorem 57.1), and negative torsion eigenvalue → negative mass-squared (the mode sits at a local maximum, not minimum, of the torsion potential).
